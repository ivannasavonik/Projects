function [trainedClassifier, validationAccuracy] = GNB_PCA_trainClassifier(trainingData, responseData)
...
% Auto-generated by MATLAB on 26-Aug-2020 10:43:12
...
trainedClassifier.predictFcn = @(x) naiveBayesPredictFcn(pcaTransformationFcn(predictorExtractionFcn(x)));
...
for fold = 1:KFolds
    ...
    pcaTransformationFcn = @(x) [ array2table((table2array(varfun(@double, x(:, ~isCategoricalPredictorBeforePCA))) - pcaCenters) * pcaCoefficients), x(:,isCategoricalPredictorBeforePCA) ];
    naiveBayesPredictFcn = @(x) predict(classificationNaiveBayes, x);
    validationPredictFcn = @(x) naiveBayesPredictFcn(pcaTransformationFcn(x));
